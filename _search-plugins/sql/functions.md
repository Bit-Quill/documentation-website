...
cbrt | `cbrt(number T) -> T` | `SELECT cbrt(0.5) FROM`
ceil | `ceil(number T) -> T` | `SELECT ceil(0.5) FROM`
...
floor | `floor(number T) -> T` | `SELECT floor(0.5)`
ln | `ln(number T) -> double` | `SELECT ln(10)`
log | `log(number T) -> double` or `log(number T, number) -> double` | `SELECT log(10, 1000)`
...
date_format | `date_format(date, string) -> string` or `date_format(date, string, string) -> string` | `SELECT date_format(date, 'Y')`
date_sub | `date_sub(date, INTERVAL expr unit) -> date` | `SELECT date_sub(date('2008-01-02'), INTERVAL 31 day)`
dayofmonth | `dayofmonth(date) -> integer` | `SELECT dayofmonth(date)`
...
timestamp | `timestamp(date) -> date` | `SELECT timestamp(date)`
to_days | `to_days(date) -> long` | `SELECT to_days(date '2008-10-07')`
week | `week(date[mode])  -> integer` | `SELECT week(date('2008-02-20'))`
year | `year(date) -> integer` | `SELECT year(date)`

## String

Function | Specification | Example
:--- | :--- | :---
ascii | `ascii(string T) -> integer` | `SELECT ascii(name.keyword)`
...
rtrim | `rtrim(string T) -> T` | `SELECT rtrim(name.keyword)`
substring | `substring(string T, integer, integer) -> T` | `SELECT substring(name.keyword, 2,5)`
...
count | `count(number T) -> T` | `SELECT count(date)`
